...
class ParklabDataPull < DataPull
  ...
  def get_trucks
    parse_feed
    data = parse_post
    if data == 'Too lazy to find out, look here yourself (honestly though, it\'s probably the case that *they* actually have been too lazy to update their menu online): http://parklabjunction.com/ or https://lmgtfy.com/?q=parklab+junction.' || data == nil
      return data || 'No menu\'s posted - see the latest here: https://www.facebook.com/ParklabJunction'
    end
    enhance_data
  end
end
